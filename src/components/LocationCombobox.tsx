    ...
    <Combobox value={value} onChange={onChange}>
      <div className={`relative w-full ${className}`}>
        <div className="relative">
          <ComboboxInput
            className={`input ${onGeolocate ? "pr-18" : "pr-10"}`}
          ...
          <div className="absolute inset-y-0 right-0 flex items-center gap-1 pr-2">
            {onGeolocate && (
              <button
                ...
                className="flex items-center justify-center rounded p-1 text-neutral-800 hover:text-primary-500 focus:text-primary-500 focus:outline-none transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isGeoLoading ? (
                  <svg
